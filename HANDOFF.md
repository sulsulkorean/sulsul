...
사람이 검수해 발행 대기열에 넣은 글만 하루 2편씩 공개됩니다.

| 언제 | 무엇 | 몇 개 | 발행 방식 |
|---|---|---|---|
| 매일 오전 9시 | 트렌드 기반 글 | 2개 | **초안만 만들고 대표님 승인 후 발행** |
| 매일 오전 10시 30분 | 검수 완료 대기 글 | 2개 | 전체 검사·빌드 후 자동 공개 |
...
- **검수 완료 글 10편 교정·대기열 등록** — `_scheduled/`에서 매일 2편 공개
...
| `.github/workflows/release_reviewed_posts.yml` | 매일 오전 검수 완료 글 2편 공개 |
| `tools/release_scheduled_posts.py` | `_scheduled/`의 앞 2편을 `_posts/`로 이동하고 발행일 갱신 |
| `src/app/what-is-sulsul/page.tsx` | 브랜드 정의 페이지 |
| `public/llms.txt`, `llms-full.txt` | AI 검색엔진용 브리핑 |
| `_posts/` | 공개된 글 |
| `_scheduled/` | 검수 완료·예약 대기 글. 파일 앞 `01-`, `02-` 순서대로 공개 |
...
- 공개 글: `_posts/maintenance.md` 안내문 1개
- 발행 대기: `_scheduled/`에 검수 완료 글 10개
- 10편 전체 검사 결과: 1,200~1,500단어, 이미지 각 2장, 제목 중복 없음, 공개 문구 검사·사이트 빌드 통과
- 발행 계획: 매일 오전 10시 30분, 2편씩 5일간 공개

### 우선순위 높음

1. **첫 예약 공개 결과 확인**
   - 오전 10시 30분 실행에서 첫 2편이 `_posts/`로 이동하는지
   - 블로그 홈·사이트맵·RSS에 두 글이 나타나는지
